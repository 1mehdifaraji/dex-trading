...
const fetchTokenHolders = async (
  ...
  const response = await fetch(
    `https://onchain.dextrading.com/rangeholders?network=${network}&till=${
      toDate.toISOString().split("T")[0]
    }&limit=20&token=${id}&minAmount=${from}&maxAmount=${to}`
  );
...
export default function TokenHolders({ tokenAddress }: Props) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["tokenHolders", tokenAddress, form.from, form.to],
    queryFn: () => fetchTokenHolders("eth", tokenAddress, form.from, form.to),
  });
    ...
    <div className="relative xl:max-w-[1200px] xl:mx-auto xl:min-w-[1200px] border border-base-content/50 rounded-lg p-4 pb-16 overflow-hidden">
      ...
      <div className="flex items-center gap-3 lg:pl-[60px] flex-wrap sm:flex-nowrap mb-5 mt-5">
        <label className="form-control w-full flex items-center justify-start gap-2">
          ...
          <Select
            onValueChange={(e) => handleSelectChange("from", +e)}
            defaultValue={form.from.toString()}
          >
        ...
        <label className="form-control  w-full flex items-center justify-start gap-2">
          ...
          <Select
            onValueChange={(e) => handleSelectChange("to", +e)}
            defaultValue={form.to.toString()}
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder="Select starting date" />
            </SelectTrigger>
              ...
              (showMore ? data : data.slice(0, 10)).map(
                (data: any, id: number) => (
                  <Addresses
                    ...
                    value={id}
                    address={data.Holder.Address}
                    haveLine={id !== data.length - 1 && id !== data.length - 2}
                  />
                )
              )}
          </div>
